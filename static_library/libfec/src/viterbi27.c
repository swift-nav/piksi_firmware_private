...
void v27_poly_init(v27_poly_t *poly, const signed char polynomial[2]) {
  ...
  for (state = 0; state < 32; state++) {
    poly->c0[state] =
        (polynomial[0] < 0) ^ parity((2 * state) & abs(polynomial[0])) ? 255
                                                                       : 0;
    poly->c1[state] =
        (polynomial[1] < 0) ^ parity((2 * state) & abs(polynomial[1])) ? 255
                                                                       : 0;
  }
}
 ...
 * \param initial_state Initial state of the decoder shift register. Usually
 * zero.
 */
void v27_init(v27_t *v, v27_decision_t *decisions, unsigned int decisions_count,
  ...
  for (i = 0; i < 64; i++) v->old_metrics[i] = 63;
...
void v27_update(v27_t *v, const unsigned char *syms, int nbits) {
  ...
  while (nbits--) {
    ...
    if (v->new_metrics[0] > (1 << 30)) {
      ...
      for (i = 0; i < 64; i++) {
        if (v->new_metrics[i] < minmetric) minmetric = v->new_metrics[i];
      }

      for (i = 0; i < 64; i++) v->new_metrics[i] -= minmetric;

      normalize += minmetric;
    }

    /* Advance decision index */
    if (++v->decisions_index >= v->decisions_count) v->decisions_index = 0;
...
void v27_chainback_fixed(v27_t *v, unsigned char *data, unsigned int nbits,
  ...
  while (nbits-- != 0) {
    ...
    decisions_index =
        (decisions_index == 0) ? v->decisions_count - 1 : decisions_index - 1;
