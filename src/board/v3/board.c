...
#define UART1_TX_USB_GPIO_LINE PAL_LINE(GPIO1, 16)
#define UART1_RX_USB_GPIO_LINE PAL_LINE(GPIO1, 17)
...
void boardInit(void)
{
  ...
  *(volatile uint32_t *)0xF8000240 = 0x0;
   ...
   * UART1 is routed to USB UART by FSBL. Disconnect and route to EMIO. */
  palSetLineMode(UART1_TX_USB_GPIO_LINE, PAL_MODE_UNCONNECTED);
  palSetLineMode(UART1_RX_USB_GPIO_LINE, PAL_MODE_UNCONNECTED);
