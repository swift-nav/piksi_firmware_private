...
void nap_track_update_wr_blocking(u8 channel,
                                  s32 carrier_freq,
                                  u32 code_phase_rate,
                                  u8 rollover_count,
                                  u8 corr_spacing) {
...
u64 nap_sample_time_to_count(u32 sample_count) { return sample_count; }

double nap_count_to_ms(u64 delta_time) { return delta_time; }

void nap_acq_code_wr_blocking(gnss_signal_t sid) { (void)sid; }

void nap_acq_load_wr_enable_blocking(void) {}

void nap_acq_init_wr_params_blocking(s16 carrier_freq) { (void)carrier_freq; }

u32 nap_error_rd_blocking(void) { return 0; }
...
void nap_pps(u64 rising_edge_count_8x) { (void)rising_edge_count_8x; }

void nap_pps_width(u32 falling_edge_count) { (void)falling_edge_count; }
