...
#include "gal_e1b_mem.h"
#include "gal_e1c_mem.h"
...
/** \fn int CodeGenGps
 *  \brief Generates the GPS PRN spreading codes
 */
int CodeGenGps(uint32_t _iSvId, int8_t *_piCode, int _iCodeLen) {
  ...
  /* Sanity checks */
  if (_iCodeLen != SOFTMACQ_GPSL1CA_CODE_CHIPS) { return -1; }
  if (_iSvId     < SOFTMACQ_GPSL1CA_PRN_BASE  ) { return -1; }
  ...
  for(i=0; i<1023; i++) {
    ...
    tmp = reg[1] * reg[2] * reg[5] * reg[7] * reg[8] * reg[9];
    for(j=9; j > 0; j--) { reg[j] = reg[j-1]; }
    reg[0] = tmp;
  }

  tmp = SOFTMACQ_GPSL1CA_CODE_CHIPS - g2s[(_iSvId-SOFTMACQ_GPSL1CA_PRN_BASE)];
  for(i=0; i<_iCodeLen; i++) {
    _piCode[i] = -g1[i] * g2[tmp];

    tmp++;
    if (tmp==_iCodeLen) tmp=0;
  }
...
/** \fn void CodeGenSbas
 *  \brief Generate the SBAS PRN spreading codes
 */
int CodeGenSbas(uint32_t svId, int8_t *_piCode, int codelen) {
  int g2s[] = { 145, 175, 52, 21, 237, 235, 886, 657, 634, 762,
  ...
  /* Sanity checks */
  if (codelen != 1023) { return -1; }
  if (svId < SOFTMACQ_SBASL1_PRN_BASE) { return -1; }
  ...
  tmp = 1023 - g2s[(svId-SOFTMACQ_SBASL1_PRN_BASE)];
  for(i=0; i<codelen; i++) {
    _piCode[i] = -g1[i] * g2[tmp];

    tmp++;
    if (tmp==codelen) tmp=0;
  }
  return 0;
}
...
static int8_t cHex2BinTable[16][4] = {
...
/** \fn int CodeGenGalE1B
 *  \brief Expands the Galileo PRN memory codes for the data channel
 */
int CodeGenGalE1B(int _iSvId, int8_t *piCode, int iCodeLen) {
  int iIdx;
  uint8_t uChar;
  /* Sanity checks */
  if (_iSvId < SOFTMACQ_GAL_SATBASE) { return -1; }
  if (iCodeLen != SOFTMACQ_GALE1B_CODECHIPS) { return -1; }

  for (iIdx=0; iIdx < iCodeLen; iIdx+=4) {
    uChar = HexToDec(GALE1BMAT[_iSvId - SOFTMACQ_GAL_SATBASE][iIdx/4]);
    piCode[iIdx+0] = cHex2BinTable[uChar][0];
    piCode[iIdx+1] = cHex2BinTable[uChar][1];
    piCode[iIdx+2] = cHex2BinTable[uChar][2];
    piCode[iIdx+3] = cHex2BinTable[uChar][3];
  }

  return 0;
}

/** \fn int CodeGenGalE1C
 *  \brief Expands the Galileo PRN memory codes for the pilot channel
 */
int CodeGenGalE1C(int _iSvId, int8_t *piCode, int iCodeLen) {
  ...
  /* Sanity checks */
  if (_iSvId < SOFTMACQ_GAL_SATBASE) { return -1; }
  if (iCodeLen != SOFTMACQ_GALE1C_CODECHIPS) { return -1; }

  for (iIdx=0; iIdx < iCodeLen; iIdx+=4) {
    uChar = HexToDec(GALE1CMAT[_iSvId - SOFTMACQ_GAL_SATBASE][iIdx/4]);
    piCode[iIdx+0] = cHex2BinTable[uChar][0];
    piCode[iIdx+1] = cHex2BinTable[uChar][1];
    piCode[iIdx+2] = cHex2BinTable[uChar][2];
    piCode[iIdx+3] = cHex2BinTable[uChar][3];
